poly_coords<- function(shapefile){
  ...
  if (nrow(data.frame(shapefile$pkuid))< 1) 
    
  {
    ...
    for(curr_row in curr_poly_start_row:num_polys)
    {
      ...
      Poly_Name<-as.vector(Poly_n$pkuid)
      Poly<-shapefile[shapefile$pkuid==Poly_Name,]
      ...
      if (poly_old != Poly_n$pkuid)
      {
        YX1<- data.frame(Poly_Name, Order, PolyCoordsY, PolyCoordsX)
        YX2<-rbind(YX3,YX1)
        YX3<-YX2
      }
      poly_old<-Poly_n$pkuid
    }
    
    join<-merge(YX3, shapefile@data, by.x="Poly_Name", by.y= "pkuid", all=T)
